...
class SafetyLayer(CommonroadEnv):
    ...
    def reset(self, seed=None, options: Optional[dict] = None, benchmark_id=None, scenario: Scenario = None,
        ...
        self.time_step = 0
        self.time_step = 0
        if int(self.scenario.scenario_id.map_name[3]) in self.cache:
            self.precomputed_lane_polygons, self.conflict_lanes, self.dense_lanes = self.cache[int(self.scenario.scenario_id.map_name[3])]
        else:
            self.compute_lane_sides_and_conflict()
            self.cache[int(self.scenario.scenario_id.map_name[3])] = self.precomputed_lane_polygons, self.conflict_lanes, self.dense_lanes
