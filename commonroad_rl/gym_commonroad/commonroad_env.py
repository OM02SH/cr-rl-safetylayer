...
# class CommonroadEnv(gym.Env):
        ...
        def load_reset_config(path):
            ...
            for p in sorted(path.glob("*.pickle")):
                with p.open("rb") as f:
                    ...
                    if not valid_scenario(problem, p.stem):
                        continue
                    problem_dict[p.stem] = problem
            return problem_dict
    ...
    def step(self, action: Union[np.ndarray, State]) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        observation_surrounding_obstacles = self.observation_collector.observe_surrounding_obstacles(self.ego_action.vehicle)
        ...
        info = {
            ...
            "detected_obstacles": observation_surrounding_obstacles
        }
        ...
        terminated = done
        truncated = False
        # NOTE: truncated is always False
