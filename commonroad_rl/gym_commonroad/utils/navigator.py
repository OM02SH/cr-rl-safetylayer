...
# from commonroad_route_planner.route_planner import RouteType
...
from commonroad_clcs.util import resample_polyline, compute_polyline_length
...
class Navigator:
    ...
    def _initialize_lane_ccosy_kd_ref_dict(self):
        ...
        for id_curvilinear, polyline in id_polyline:
            # print("initialize lane_ccosy_kd_ref_dict: ", id_curvilinear)
            ...
            # plt.scatter(x, y)
            # plt.title("polyline before resample")
            # plt.xlabel("X axis")
            # plt.ylabel("Y axis")
            # plt.show()


            if abs(ccosy.curvilinear_projection_domain()[0][1]) < 39.9:
                # print("why??", ccosy.curvilinear_projection_domain()[0][1])
                # print("retry with method 2")
                ccosy = pycrccosy.CurvilinearCoordinateSystem(resampled_polyline, default_projection_domain_limit=40.0,  method=2)

                if abs(ccosy.curvilinear_projection_domain()[0][1]) < 39.9:
                    print("still failed")
                    np.save("resampled_polyline.npy", resampled_polyline)
                    print("saved resampled_polyline.npy")
    ...
    def create_coordinate_system_from_polyline(
        ...
        ccosy = pycrccosy.CurvilinearCoordinateSystem(resampled_polyline, default_projection_domain_limit=40.0)
    ...
    def _get_safe_curvilinear_coords(
        ...
        try:
            ...
            if not ccosy.cartesian_point_inside_projection_domain(position[0], position[1]):
    ...
    def _get_curvilinear_coords(self, ccosy, position: np.ndarray) -> np.ndarray:
        # version 06.2021
        # if not ccosy.cartesian_point_inside_projection_domain(position[0], position[1]):
            # points = ccosy.projection_domain()
            # # points = ccosy.projection_domain()
            # # 将 x 和 y 坐标从 ndarray 中提取出来
            ...
            # # 添加标题和坐标轴标签
            # plt.title("ccosy projection domain")
            # plt.xlabel("X axis")
            # plt.ylabel("Y axis")
    ...
    def _get_curvilinear_coords_over_lanelet(self, lanelet: Lanelet, position):
        # version 2020
        # print("get curvilinear coords over lanelet")
    ...
    def _get_safe_distance_to_curvilinear2(
        ...
        # print("ego vehicle cartesian is in projection domain: ", ccosy.cartesian_point_inside_projection_domain(position_ego[0], position_ego[1]))
        
        # if not ccosy.cartesian_point_inside_projection_domain(position_ego[0], position_ego[1]):
        #     points = ccosy.curvilinear_projection_domain()
        #     # points = ccosy.projection_domain()
        #     # 将 x 和 y 坐标从 ndarray 中提取出来
        #     x = [point[0] for point in points]
        #     y = [point[1] for point in points]
        ...
        indomain = 1.0
        # try:
        if ccosy.cartesian_point_inside_projection_domain(position_ego[0], position_ego[1]):
            # print("id_curvilinear: ", id_curvilinear, "position_ego: ", position_ego)
    ...
    def safe_convert_to_cartesian_coords(
        ...
        try:
            ...
            eps = 0.1
            # without this +0.1 can't convert_to_cartesian_coords constantly
            # long_clcs_eval = min(max(long_lat_eval[0], eps), length_ccosy - eps)+0.1
            long_clcs_eval = min(max(long_lat_eval[0], eps), length_ccosy - eps)
            lat_clcs_eval = min(max(long_lat_eval[1], -max_lat + eps), max_lat)
            # print("length_ccosy, max_lat, eps: ", length_ccosy, max_lat, eps)
            # print("long_lat_eval, long_clcs_eval, lat_clcs_eval: ", long_lat_eval, long_clcs_eval, lat_clcs_eval)
            # print("ccosy.curvilinear_projection_domain() 2: ", ccosy.curvilinear_projection_domain())
    ...
    def get_points_on_ccosy(
        ...
        # print("distances_ref_path: ", distances_ref_path)
        ...
        for distance_ref in distances_ref_path:
            ...
            # print("p_curvilinear, distance_ref: ", p_curvilinear, distance_ref)
            long_clcs_eval = p_curvilinear + distance_ref
            # make valid in ccosy
            # print("ccosy.curvilinear_projection_domain(): ", ccosy.curvilinear_projection_domain())
            # points = ccosy.curvilinear_projection_domain()
            # # 将 x 和 y 坐标从 ndarray 中提取出来
            # x = [point[0] for point in points]
            # y = [point[1] for point in points]
            ...
            # # 添加标题和坐标轴标签
            # plt.title("Scatter Plot of Points from ndarray")
            # plt.xlabel("X axis")
            # plt.ylabel("Y axis")
