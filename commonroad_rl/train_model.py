...
class AdaptiveParamNoiseSpec:
    def __init__(self, initial_stddev, desired_action_stddev):
    ...
    def update_stddev(self, current_action_stddev):
        """ 根据当前动作标准差自适应更新噪声标准差 """
        if current_action_stddev > self.desired_action_stddev:
            self.stddev *= 0.99  # 降低噪声
        else:
            self.stddev *= 1.01  # 增加噪声
...
def create_parameter_schedule(hyperparams: dict):
    ...
    for key in ["learning_rate", "cliprange", "cliprange_vf"]:
        ...
        if isinstance(hyperparams[key], str):
            ...
            hyperparams[key] = lambda _: float(hyperparams[key])
        else:
...
def run_stable_baselines(args):
    ...
    def create_env(n_envs, eval_env=False, **kwargs):
        ...
        if is_atari:
            ...
            new_env = make_atari_env(env_id, n_env=n_envs, seed=args.seed)
