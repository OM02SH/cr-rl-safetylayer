...
def load_results(root_dir_or_dirs, enable_progress=True, enable_monitor=True, verbose=False):
    ...
    enable_progress: bool - if True, will attempt to load data from progress.csv files (data saved by logger).
    Default: True

    enable_monitor: bool - if True, will attempt to load data from monitor.csv files (data saved by Monitor
    environment wrapper). Default: True
            ...
            if set(['metadata.json', 'monitor.json', 'progress.json', 'progress.csv']).intersection(files) or any(
                ...
                if enable_progress:
                    if osp.exists(progjson):
                        ...
                        with open(progjson, "r") as fh:
                            json_data = json.load(fh)
                        result['progress'] = pandas.DataFrame(json_data)
                    elif osp.exists(progcsv):
                        try:
                            # result['progress'] = read_csv(progcsv)
                            result['progress'] = pandas.read_csv(progcsv)
...
def plot_results(
    ...
    average_group: bool                     - if True, will average the curves in the same group and plot the mean.
    Enables resampling
                                              (if resample = 0, will use 512 steps)

    shaded_std: bool                        - if True (default), the shaded region corresponding to standard
    deviation of the group of curves will be
                                              shown (only applicable if average_group = True)

    shaded_err: bool                        - if True (default), the shaded region corresponding to error in mean
    estimate of the group of curves
                                              (that is, standard deviation divided by square root of number of
                                              curves) will be
                                              shown (only applicable if average_group = True)

    figsize: tuple or None                  - size of the resulting figure (including sub-panels). By default,
    width is 6 and height is 6 times number of
                                              sub-panels.
    ...
    Resampling is performed via symmetric
                                              EMA smoothing (see the docstring for symmetric_ema).
                                              Default is zero (no resampling). Note that if average_group is True,
                                              resampling is necessary; in that case, default
                                              value is 512.

    smooth_step: float                      - when resampling (i.e. when resample > 0 or average_group is True),
    use this EMA decay parameter (in units of the new grid step).
        ...
        for result in sresults:
            ...
            if average_group:
                ...
                if resample:
                    ...
                    ystd = np.array(ystd)
                    ystderr = ystd / np.sqrt(len(y))  # np.array(ystderr)
                    y = ymean
                    x = xs
...
#        print(np.min(x)-0.005)
        ...
        if legend:
            if label is not None:
                ...
                s = f.subplotpars
                # bb = [s.left+0.2, s.top+0.02, s.right-s.left - 0.6, 0.05] #
                bb = [s.left + 0.05, s.top + 0.25, s.right - s.left + 1.0, 0.05]
        ...
        if xlabel is not None:
            plt.sca(axarr[idx_row][idx_col])
            plt.xlabel(xlabel)  # for ax in axarr[-1]:  #     plt.sca(ax)  #     plt.xlabel(xlabel)
        ...
        if ylabel is not None:
            ...
            if labelpad is not None:
                ...
                plt.ylabel(ylabel)  # for ax in axarr: #[:, 0]:  #     plt.sca(ax)  #     plt.ylabel(ylabel)
